...
const AdminCreateAssessmentTestAgePage = () => {
    ...
    const successText = "data kategori training berhasil disimpan"

    const handleNextCategory = (input) => {
        const list = {};
        list.contents = input;
        list.category_name = "training";
        if(data.length === 0){
            createData(list, "questions", successText);
        }else{
            list.category_name = "training";
            updateData(list, "questions", successText, data[2]?._id);
        }
        navigate("/admin/create-assessment-test-4");
    }    
    ...
    //     await setSearch("training", "category_name", "equals");
    //     getData();
    // }

    // const getRecord = () => {
    //     getRecords()
    //     setInputList(data.contents)
    // }
